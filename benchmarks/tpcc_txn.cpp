...
RC tpcc_txn_man::run_payment(tpcc_query * query) {
	...
	assert(item != NULL);
    // cly begin: check where location point to
	row_t * r_wh_local;
    if((uint64_t)item->location & DRAM_TAG){ // dram_row
        row_t * r_wh = ((row_t *)((uint64_t)item->location & DRAM_MASK));
	    if (g_wh_update)
		    r_wh_local = get_row(r_wh, WR, index, key);
	    else 
		    r_wh_local = get_row(r_wh, RD, index, key);
    } else { // nvm_row
        nvm_row * r_wh = ((nvm_row *)item->location);
    	if (g_wh_update)
		    r_wh_local = get_row(r_wh, WR, index, key, wh_to_part(w_id));
	    else 
		    r_wh_local = get_row(r_wh, RD, index, key, wh_to_part(w_id));
    }
    // cly end
	// cly delete: row_t * r_wh = ((row_t *)item->location);
    ...
    row_t * r_dist_local;
    if((uint64_t)item->location & DRAM_TAG){ // dram_row
        row_t * r_dist = ((row_t *)((uint64_t)item->location & DRAM_MASK));
	    r_dist_local = get_row(r_dist, WR, _wl->i_district, key);
    } else {
        nvm_row * r_dist = ((nvm_row *)item->location);
	    r_dist_local = get_row(r_dist, WR, _wl->i_district, key, wh_to_part(w_id));
    }
    // cly end
	// cly delete: row_t * r_dist = ((row_t *)item->location);
	...
	row_t * r_cust_local;
    // cly begin: move key and index out of the if sentence
	//uint64_t key;
    //INDEX * index;
    if (query->by_last_name) { 
        ...
        index->index_release_latch(key, wh_to_part(c_w_id));
		assert(item != NULL);
        ...
        // cly begin
        uint64_t pri_key = (uint64_t) mid->location;
        //printf("pri_key = %lu\n", pri_key);
        index = _wl->i_customer_id;
        itemid_t * pri_item = index_read(index, pri_key, wh_to_part(c_w_id));
        assert(pri_item != NULL);
        // check where location point to
        if((uint64_t)pri_item->location & DRAM_TAG){ // dram_row
            row_t * r_cust = ((row_t *)((uint64_t)pri_item->location & DRAM_MASK));
	        r_cust_local = get_row(r_cust, WR, index, pri_key);
        } else { // nvm_row
            nvm_row * r_cust = ((nvm_row *)pri_item->location);
	        r_cust_local = get_row(r_cust, WR, index, pri_key, wh_to_part(c_w_id));
        }
        // cly end
		// cly delete: r_cust = ((row_t *)mid->location);
        ...
        // cly begin: check where location point to
        if((uint64_t)item->location & DRAM_TAG){ // dram_row
            row_t * r_cust = (row_t *)((uint64_t)item->location & DRAM_MASK);
	        r_cust_local = get_row(r_cust, WR, index, key);
        } else { // nvm_row
            nvm_row * r_cust = (nvm_row *)(item->location);
	        r_cust_local = get_row(r_cust, WR, index, key, wh_to_part(c_w_id));
        }
        // cly end
		// cly delete: r_cust = (row_t *) item->location;
	}
...
RC tpcc_txn_man::run_new_order(tpcc_query * query) {
    ...
    if((uint64_t)item->location & DRAM_TAG){ // dram_row
        row_t * r_cust = ((row_t *)((uint64_t)item->location & DRAM_MASK));
	     r_cust_local = get_row(r_cust, RD, index, key);
    } else { // nvm_row
        nvm_row * r_cust = ((nvm_row *)item->location);
	    r_cust_local = get_row(r_cust, RD, index, key, wh_to_part(w_id));
    }
    // cly end
    // cly delete: row_t * r_cust = (row_t *) item->location;
	if (r_cust_local == NULL) {
		return finish(Abort); 
	}
	uint64_t c_discount;
    ...
    // cly add
    index_insert(_wl->i_order, orderPrimaryKey(w_id, d_id, o_id), r_order, wh_to_part(w_id));
	/*=======================================================+
    EXEC SQL INSERT INTO NEW_ORDER (no_o_id, no_d_id, no_w_id)
        VALUES (:o_id, :d_id, :w_id);
    +=======================================================*/
    // cly uncomment: though there is no index for new_order
    // which means you can never get to these rows any more
	row_t * r_no = NULL;
	_wl->t_neworder->get_new_row(r_no, 0, row_id);
    r_no->init_manager(r_no);
	...
	for (UInt32 ol_number = 0; ol_number < ol_cnt; ol_number++) {
        ...
        row_t * r_item_local;
        if((uint64_t)item->location & DRAM_TAG){ // dram_row
            row_t * r_item = ((row_t *)((uint64_t)item->location & DRAM_MASK));
		    r_item_local = get_row(r_item, RD, _wl->i_item, key);
        } else { // nvm_row
            nvm_row * r_item = ((nvm_row *)item->location);
		    r_item_local = get_row(r_item, RD, _wl->i_item, key, 0);
        }
        // cly end
		// cly delete: row_t * r_item = ((row_t *)item->location);
        ...
        // cly begin: check where location point to
        row_t * r_stock_local;
        if((uint64_t)stock_item->location & DRAM_TAG){ // dram_row
            row_t * r_stock = (row_t *)((uint64_t)stock_item->location & DRAM_MASK);
		    r_stock_local = get_row(r_stock, WR, stock_index, stock_key);
        } else { // nvm_row
            nvm_row * r_stock = (nvm_row *)(stock_item->location);
		    r_stock_local = get_row(r_stock, WR, stock_index, stock_key, wh_to_part(ol_supply_w_id));
        }
        // cly end
		// cly delete: row_t * r_stock = ((row_t *)stock_item->location);
        ...
        r_ol->init_manager(r_ol);
        r_ol->set_primary_key(orderlineKey(w_id, d_id, o_id, ol_number));
        ...
        // cly add
        index_insert(_wl->i_orderline, orderlineKey(w_id, d_id, o_id, ol_number), r_ol, wh_to_part(w_id));
#endif
	}
	assert( rc == RCOK );
	return finish(rc);
